...
# 核心破解逻辑：直接伪装请求避开封锁，改用无 Cookie 墙的 v7 quote 接口
def fetch_stock_internal(ticker: str) -> dict:
    ...
    try:
        # 直接使用 v7 quote 接口，避开 Cookie 验证模块，一次性拿齐价格和 PE
        ...
        if not res_list:
            raise ValueError("Target not found or completely blocked")
        ...
        # 提取 PE (市盈率) 和全名
        pe_val = quote.get('trailingPE')
        pe_ratio = str(round(pe_val, 2)) if pe_val is not None else "N/A"
        stock_name = quote.get('longName') or quote.get('shortName') or symbol_str
